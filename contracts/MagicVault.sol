...
contract MagicVault is OwnableUpgradeSafe {
    ...
    // Update the given pool's MAGICs allocation point. Can only be called by the owner.
        // Note contract owner is meant to be a governance contract allowing MAGIC governance consensus
    ...
    function pendingMagic(uint256 _pid, address _user)
        public
        view
        returns (uint256)
    {
    ...
    // ----
    // Function that adds pending rewards, called by the MAGIC token.
    // ----
    uint256 private magicBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = magic.balanceOf(address(this)).sub(magicBalance);

        if(newRewards > 0) {
            magicBalance = magic.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 magicRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        magicRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 magicRewardFee = magicRewardWhole.mul(DEV_FEE).div(10000);
        uint256 magicRewardToDistribute = magicRewardWhole.sub(magicRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(magicRewardFee);

        pool.accMagicPerShare = pool.accMagicPerShare.add(
            magicRewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to MagicVault for MAGIC allocation.
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accMagicPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from MagicVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid, from); // Update balances of from this is not withdrawal but claiming MAGIC farmed
    ...
    function updateAndPayOutPending(uint256 _pid, address from) internal {
        ...
        uint256 pending = pendingMagic(_pid, from);

        if(pending > 0) {
            safeMagicTransfer(from, pending);
        }
    ...
    function safeMagicTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > magicBal) {
            ...
            magicBalance = magic.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 magicBal = magic.balanceOf(address(this));
        if (pending_DEV_rewards > magicBal) {

            magic.transfer(devaddr, magicBal);
            magicBalance = magic.balanceOf(address(this));

        } else {

            magic.transfer(devaddr, pending_DEV_rewards);
            magicBalance = magic.balanceOf(address(this));
    ...
    // allowing MAGIC governance token holders to do this functions.
