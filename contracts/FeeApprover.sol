...
contract FeeApprover is OwnableUpgradeSafe {
    ...
    function setMagicVaultAddress(address _magicVaultAddress) public onlyOwner {
        magicVaultAddress = _magicVaultAddress;
        noFeeList[magicVaultAddress] = true;
    }
    ...
    function sync() public returns (bool lastIsMint, bool lpTokenBurn) {
        ...
        lastIsMint = _balanceMAGIC > lastSupplyOfMagicInPair && _balanceWETH > lastSupplyOfWETHInPair.add(minFinney.mul(1 finney));

        lastSupplyOfMagicInPair = _balanceMAGIC;
    ...
    function calculateAmountsAfterFee(
        ...
        ) public  returns (uint256 transferToAmount, uint256 transferToFeeDistributorAmount)
        {
            ...
            if(sender == tokenUniswapPair) {
                ...
                // And addition of sync to all MagicVault transactions to remove 99.99% of the cases.
                require(lastIsMint == false, "Liquidity withdrawals forbidden");
                require(lpTokenBurn == false, "Liquidity withdrawals forbidden");

            }

            if(noFeeList[sender]) { // Dont have a fee when magicvault is sending, or infinite loop
